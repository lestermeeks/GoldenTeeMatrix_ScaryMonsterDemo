...
while True:
    ...
    if NOW - TIME_OF_LAST_MOVE_EVENT > MOVE_EVENT_DURATION:
        ...
        if MOVE_STATE:                # Starting a new move?
            MOVE_EVENT_DURATION = random.uniform(0.08, 1.25) # Move time
            ANGLE = random.uniform(0, math.pi * 2)
            MAG = random.uniform(0, 1)
            EYE_NEXT = (math.cos(ANGLE) * EYE_RANGE[0] * MAG, # (0,0) in center,
                        math.sin(ANGLE) * EYE_RANGE[1] * MAG) # NOT pixel coords
    ...
    if NOW - TIME_OF_LAST_BLINK_EVENT > BLINK_EVENT_DURATION:
        ...
        if BLINK_STATE == 1:           # Starting a new blink (closing)
            BLINK_EVENT_DURATION = random.uniform(0.03, 0.1)
        elif BLINK_STATE == 2:         # Starting de-blink (opening)
            BLINK_EVENT_DURATION *= 2
            TARGET_SQUINT_RATIO = random.uniform(0, 0.5)
        else:                          # Blink ended,
            BLINK_STATE = 0            # paused
            BLINK_EVENT_DURATION = random.uniform(BLINK_EVENT_DURATION * 3, 10)

    if BLINK_STATE: # Currently in a blink?
        # Fraction of closing or opening elapsed (0.0 to 1.0)
        #RATIO = TARGET_SQUINT_RATIO + (( 1 - TARGET_SQUINT_RATIO ) * (NOW - TIME_OF_LAST_BLINK_EVENT) / BLINK_EVENT_DURATION)
        if BLINK_STATE == 2:    # opening 1 -> target
            RATIO = 1 - (( 1 - TARGET_SQUINT_RATIO ) * (NOW - TIME_OF_LAST_BLINK_EVENT) / BLINK_EVENT_DURATION)
        else:
            RATIO = TARGET_SQUINT_RATIO + (( 1 - TARGET_SQUINT_RATIO ) * (NOW - TIME_OF_LAST_BLINK_EVENT) / BLINK_EVENT_DURATION)
    else:           # Not blinking
        RATIO = TARGET_SQUINT_RATIO
